...
main() {
    int a = -23;
    long int b = 0;
    bool signal[50] = {0}; // начинаем с 1-ого элемента
    perevod_10_2(signal, 42, 49, a); // включая концы
    perevod_2_10(signal, 42, 49, b); // включая концы
    for(int i = 1; i < 50; i++) cout << signal[i];
    cout << endl << b;
}
