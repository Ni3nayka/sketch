...
int input() {
    ...
    getline(cin, S);
    int i = 0, a = 0;
    for (; probel(S[i]); i++) if (S[i+1] == 0) a = -1;
...
bool file_test(int a1, int a2, int a3, int a4) { // marks student subject output
    ...
    if (E) cout << "ATTENTION: to use this program, please correct file errors" << endl;
    return E;
}
...
void function_3_Get_Average_Mark_input_subject() {
    cout << "function 3: Get Average Mark" << endl;
    if (!file_test(2,0,2,0)) {
        ifstream marks;   // marks.open("marks.txt");     marks.close();
        ifstream subject; // subject.open("subject.txt"); subject.close();
        subject.open("subject.txt");
        string S;
        cout << "if you want to go out, enter <0>" << endl;
        cout << "enter subject number:" << endl;
        while (getline(subject, S)) cout << S << endl;
        subject.close();
        int s = 0;
        int l = 0;
        do {
            ...
            if (s != 0) {
                ...
                subject.close();
                if (fl) {
                    cout << "ERROR: This subject does not exist" << endl;
                    s = -1;
                }
            }
        } while ((s < 0));
        if (s != 0) {
            ...
            subject.close();
            if (sum_i > 0) {
                cout << "average marks for this subject: " << sum/sum_i << endl;
            }
            else cout << "no marks for this subject" << endl;
        }
    }
    cout << "exit function" << endl;
}

void function_5_Get_sort_output_file() {
    cout << "function 5: Get sort output file" << endl;
    //if (marks && student && subject && output) {
    if (!file_test(2,1,2,1)) {
        file_test(0,2,0,0);
        ifstream marks;   // marks.open("marks.txt");     marks.close();
        ...
        //cout << "mas_s (quantity elements): " <<mas_s.size() << endl; // => 0
        int p = 0;
        vector <int> mas_p; // numeracia studets po alfavity
        mas_p.push_back(0);
        typedef vector<char> vchar;
        vector <vchar> A;
        student.open("student.txt");
        if (true) {
            int j = 0;
            while (getline(student, S)) { // input in array
                int i = 0;
                for(; probel(S[i]); i++);
                if (S[i] != 0) {
                    A.resize(j+1);
                    ...
                    for (; y < D-5; y++) A[j].resize(y+1);
                    j++;
                }
            }
        }
        student.close();
        //for(int i = 0; 0; i++) { // i <= A[i].size()
            for(int j = 0; j < A.size(); j++) {
                for(int y = 0; y < A.size() - 1; y++) {
                    if ((mg(A[y+1][0]))%100 < (mg(A[y][0]))%100) {
                        int t = mas_p[y+1];
                        mas_p[y+1] = mas_p[y];
                        mas_p[y] = t;
                        for (int q = 0; q < A[y].size(); q++) {
                            char f = A[y][q];
                            A[y][q] = A[y+1][q];
                            A[y+1][q] = f;
                        }
        ...
        //}
        /*for (int j = 0; j < A.size(); j++) { // output in array
            int N =  mas_p[j];
            cout << N << " >";
...
void function_6_Input_new_record__choose_file() {
    ...
    do {
        cout << "==> ";
        n_file = input();
        if ((n_file < 0) || (n_file > 3)) cout << "ERROR: This command does not exist" << endl;
    ...
    if (n_file) {
        ...
        cout << "if ypu want to edit the line in the file, then enter <2>" << endl;
        cout << "if you want to escape, then enter <0>" << endl;
        int operation = 0;
        do {
            cout << "=> ";
            operation = input();
            if ((operation < 0) || (operation > 2)) cout << "ERROR: This command does not exist" << endl;
            ...
            if (ifile) {
                ...
                while (flag) {
                    ...
                    do {
                        ...
                        getline(cin, S);
                        int i = 0;
                        for (; probel(S[i]); i++) if (S[i+1] == 0) a = 0;
                        for (; num(S[i]); i++) a = a*10 + number(S[i]);
                        for (; probel(S[i]); i++);
                        if ((i == 0) || (S[i] != 0)) a = 0;
                        for (i = 0; S[i] != 0; i++) if (S[i] == '#') a = 1;
                        if (!a) cout << "ERROR: This line does not exist" << endl;
                    } while (!a);
                    for (int i = 0; S[i] != 0; i++) if (S[i] == '#') flag = 0;
                    if (flag) {
                        int l = 0;
                        for (int i = 0; S[i] != 0; i++) l = l*10 + number(S[i]);
                        cout << "> text: ";
                        getline(cin,S);
                        for (int i = 0; S[i] != 0; i++) if (S[i] == '#') flag = 0;
                        if (flag) {
                            if (n_file == 1) ofile.open("marks.txt");
                            if (n_file == 2) ofile.open("student.txt");
                            if (n_file == 3) ofile.open("subject.txt"); // <<<=========================================== uncomment
                            for (int i = 0; i < A.size(); i++ ) {
                                if (i+1 != l) {
                                    for (int j = 0; j < A[i].size(); j++ )
                                        ofile << A[i][j]; // <<<========================================================== cout => ofile
...
void function_7_Delete_record() {
    cout << "function 7: Delete record" << endl;
    if (!(file_test(2,1,2,1))) {
        file_test(2,2,2,1);
            ...
            if (ifile) {
                ...
                while (flag) {
                    ...
                    do {
                        cout << "> ";
                        t = input();
                        if (t < 0) cout << "ERROR: This line does not exist" << endl;
                    } while (t < 0);
                    if (!(t > 0)) flag = 0;
                    if (flag) {
...
main() {
    ...
    cout << "enabled functions: 1, 2, 3, 5, 6, 7;" << endl;
    ...
    do {
        ...
        else if (a == 3) function_3_Get_Average_Mark_input_subject();
        //else if (a == 4)
        else if (a == 5) function_5_Get_sort_output_file();
        ...
        else cout << "ERROR: This function does not exist" << endl;
    } while(a != 0);
    cin.get();
}
