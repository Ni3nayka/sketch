...
// LED: ==>  0..LED_GROUP_STAR,0..LED_DEFAULT_STAR
...
void loop() {
  ...
  for (int i = 0; i<LED_GROUP_STAR+1; i++) {
    ...
    if (i==LED_GROUP_STAR) {
      for (int j = LED_GROUP_STAR+1; j<LED_NUM; j++) {
        leds[j].setRGB(led_brightness[LED_GROUP_STAR][0], led_brightness[LED_GROUP_STAR][1], led_brightness[LED_GROUP_STAR][2]);
      }
    }
  }
  FastLED.show();
