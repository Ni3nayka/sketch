...
#define LIN    900 //420 // 620 // время для одной клетки (lin)
...
void linia() { // линия до центра клетки
  if (ROOT == 1) {
    ...
    k1 = k;
    //k = 0;
    while ((in1 > pr1) && (in4 > pr4) && ((in2 < pr2) || (in3 < pr3)) && (k1 + LIN > k))  { // (abs(D) < 4500) &&    && (k + LIN > k1)   && (k < LIN) 
      linia_d();
