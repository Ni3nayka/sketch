...
void forward_long(boolean flag) { // для участков дороги, гдке может быть горка
  ...
  boolean in3 = 0, Flag = 1;
  boolean F = 1;
  unsigned long int time = millis(); // смотрим, сколько времени прошлос запуска контроллера (в милисекундах) 
  if (dist()) F = 0;
  while (((in1 > M/2) || (in2 > M/2)) && Flag) {
    dat(in1, in2, in3);
    if (in3 && flag && (in1 > M/1.05) && (in2 > M/1.05) || (dist() && F)) Flag = 0;
  ...
  // предполоим, что на полное прохождение участка дороги требуется более 3 секунд
  if (time + 3000 < millis()) { // если мы проехали участок слишком быстро => мы значит заехали на горку то
    startMotor(1, M); // проезжаем ее
    startMotor(2, M);
    delay(1000);  // за 1 сек
    forward(flag); // еще раз едем по линии, до следующего участка
  }
  startMotor(1, 0);
  startMotor(2, 0);
  delay(1000);
