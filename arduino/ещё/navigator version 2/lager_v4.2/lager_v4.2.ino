...
void start() {
  ...
  while (f == 0) {//---------------------------------------------
    ...
    if (A == 4) {} // ничего, потому-что не появляется новых направлений
    if (A == 5) {
      AL();
      AR();
    }
...
void AR() {
  if (napr == 1) {  
        newt = 1; while ((!((ver[newt][1] == x) && (ver[newt][2] == y + 1))) && (newt < 35)) { newt++; } // ищем точку с нужной координатой
      ...
      if (napr == 2) {
        newt = 1; while ((!((ver[newt][1] == x - 1) && (ver[newt][2] == y))) && (newt < 35)) { newt++; } // ищем точку с нужной координатой
        if (newt > 33) { // небыло такой точки раньше
          ver[j][0] = j; 
          ver[j][1] = x - 1;
          ver[j][2] = y;
