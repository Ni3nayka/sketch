...
void updateCam() {
  ...
  ball.found = checkTimer(ball.t, 100);
  home.found = checkTimer(home.t, 100);
  target.found = checkTimer(target.t, 100);
}
...
void setSpeed(byte port, short motor_speed) {
  if (motor_speed > 255)
    ...
    motor_speed = -255;
//
//  if ((port == 1 || port == 2) && motor_speed < 0)
//    motor_speed = -255; 
...
void gleb_logic() {
  ...
  dir = home.dir;
  speed = 0;
  if (home.dist < 45) { // я в home еду вперед speed = 100
    dir = 0;
    speed = 100;
  }

  if (ball.dist > 80) { // до мяча далеко, невижу мяч
    ball.found = false;
  }

  if (ball.found && abs(ball.dir) > 0.1) {  // если видим мяч, и он не передо мной, то подстраиваемся под мяч (у ворот?)  //  && abs(ball.dir) 
   // heading = ball.dir;
    dir = sign(ball.dir) * PI / 2;
    speed = 200 + abs(ball.dir) * 400;
  }
  if (home.dist > 60) {  // если home.dist > 60 едем в home
    dir = home.dir;
    speed = 300;
  }

  if (ball.found && ball.dist < 70 && home.dist < 70 && (home.dist < 50 ? abs(ball.dir) < PI / 3 : abs(ball.dir) < PI / 2)) {      // 
    dir = ball.dir;
    speed = 340;
  }
  if (!home.found || (home.found && abs(home.dir) < 1.9)) { // если не вилим мяч или (вилим мяч && abs(home.dir) < 1.9) то goal_timer = millis()
    goal_timer = millis();
  }

  if (millis() - goal_timer < 300) {
    dir = 0;
    speed = 300;
  }
  ...
  if (!speed && abs(home.dir - PI) > 0.3) {
    dir = sign(home.dir) * PI / 2;  // sign - если знак + то 1 интаче 0
    speed = 150;
  }
}
...
void my_logic() {
  if ((home.found) && (home.dist < 80)) {
    if ((ball.found) && (ball.dist < 55)) {
      dir = ball.dir;
    }
    /*else if () { // не выбивать мяч сильно скраю
      
    }*/
    else if ((ball.found) && (ball.dist < 70)) {
      if (abs(ball.dir) < PI/9) { // мяч перед роботом
        if (ball.dist < 43) { // мяч в лунке
          dir = 0;
        }
        else {
          speed = 0;
    ...
    if (abs(home.dir) > PI*7/8) { // ворота были где-то сбоку
      dir = 0;
    }
    else { // ворота были сзади
      dir = -PI;
    }
  }
}
...
void loop() {
  ...
  setLED(lights, ball.found, home.found);
  move();
}
