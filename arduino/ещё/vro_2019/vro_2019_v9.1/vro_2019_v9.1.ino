...
#define LIN    850 // 750 //900 // время для одной клетки (lin)
...
void linia() { // линия до центра клетки
  if (ROOT == 1) {
    ...
    D_flag = 0;
    FFF = 0;
    if ((A == 2.1) || (A == 2.2)) { k = k - LIN*DUG_K; }
    //while ((in1 > pr1) && (in4 > pr4) && ((in2 < pr2) || (in3 < pr3)) && (k1 + LIN > k))  {
    while ((analogRead(A1) > pr1) && (analogRead(A4) > pr4) && ((analogRead(A2) < pr2) || (analogRead(A3) < pr3)) && (k + LIN + FFF > millis()))  {  //   && (k1 + LIN > k)
      //dat();
      linia_d();
      if ((analogRead(A0) > pr0) && (e > 0)) { D_flag++; }
      if ((analogRead(A0) > pr0) && (e < 0)) { D_flag--; }
      //FFF = abs(D_flag)/4;
      //FFF = FFF + abs(D_flag)/4;
      //if (analogRead(A0) > pr0) { D_flag = D_flag + 10; }
      //else if (D_flag > 0) { D_flag--; }
      //k = millis();
    }
    ...
    if ((A != 1) && (A != 4)) {  // (D_flag >= 3000) && //   && (A != 2.1) && (A != 2.2)
      analogWrite(PWMA, M);
      digitalWrite(AIN1, 1);
      digitalWrite(AIN2, 0);
      analogWrite(PWMB, M);
