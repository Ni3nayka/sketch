...
void dvigenie() {
  ...
  if (y == 0) {} else {
    ...
    // 1 черная
    // 0 белая
    while ((y1 > -80) || (y2 > -80)) {   // ((y1 > -80) || (y2 > -80))
      if (((analogRead(A1)) > 600) && (f1 == 1) && (y1 > 0)) { f1 = 0;       }
      if (((analogRead(A1)) < 600) && (f1 == 0) && (y1 > 0)) { f1 = 1; y1--; }
      //if (y1 > 5) { M1.setSpeed(255); } else { M1.setSpeed(180 + 15*y1); }
      M1.setSpeed(100 + 15*y1);
      if ((y1 == 0) && ((analogRead(A1)) < 600)) { y1--; } 
      if (y1 == -1) {
        if (y > 0) { M1.run(BACKWARD); }
        else { M1.run(FORWARD); }
        y1--;
      }
      if (((analogRead(A1)) > 250) && (y1 == -2)) { M1.setSpeed(  (((analogRead(A1)) - 250) / 1.5)  ); }// (((analogRead(A1)) - 250) / 1.5)   
      if (((analogRead(A1)) < 250) && (y1 == -2)) { M1.setSpeed(RELEASE); y1 = -100; } 
      /*
      if (y1 == 0) {
        M1.setSpeed(255);
        if (y > 0) { M1.run(BACKWARD); }
      ...
      */
      ...
      //if (y2 > 5) { M2.setSpeed(255); } else { M2.setSpeed(180 + 15*y2); }
      M1.setSpeed(100 + 15*y1);
      if (y2 == 0) { M2.run(RELEASE); }
      /*
      if (y2 == 0) {
        M2.setSpeed(255);
